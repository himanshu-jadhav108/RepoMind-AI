...
export function SmartLearningPanel({ runId, filePath }: SmartLearningPanelProps) {
  ...
  return (
    <div className="rounded-xl border border-border bg-card p-4 space-y-4 font-sans selection:bg-copper selection:text-white">
      {/* Header */}
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3 border-b border-border pb-3">
        ...
        <div className="flex items-center gap-1 bg-background p-1 rounded-lg border border-border font-mono text-xs">
      ...
      {loading ? (
        ...
        <div className="space-y-4 font-mono">
          {/* Overview */}
          <div>
            <h4 className="text-xs font-semibold text-emerald-600 dark:text-emerald-400 uppercase tracking-wider mb-1 flex items-center gap-1.5 font-display">
              <BookOpen className="w-3.5 h-3.5" /> Conceptual Overview
            </h4>
            <p className="text-xs text-foreground/90 leading-relaxed font-sans bg-background p-3 rounded-lg border border-border">
          ...
          {/* Tech Stack Badges */}
          <div>
            <h4 className="text-xs font-semibold text-category-arch uppercase tracking-wider mb-1 font-display">
              Technologies & Libraries Used
            </h4>
            <div className="flex flex-wrap gap-1.5">
              {explanation.tech_stack.map((tech, i) => (
                <Badge key={i} variant="outline" className="border-category-arch/30 text-category-arch bg-category-arch/10 text-[11px] font-mono">
                  {tech}
                </Badge>
              ))}
            </div>
          ...
          <div>
            ...
            <div className="space-y-1.5 text-xs">
              {explanation.key_concepts.map((kc, i) => (
                <div key={i} className="p-2.5 rounded-lg bg-background border border-border text-foreground/90">
                  {kc}
                </div>
              ))}
            </div>
            ...
            <div>
              ...
              <div className="space-y-1 text-xs">
                {explanation.best_practices.map((bp, i) => (
                  <div key={i} className="p-2 rounded bg-emerald-500/10 border border-emerald-500/20 text-emerald-600 dark:text-emerald-400">
                    {bp}
                  </div>
                ))}
              </div>

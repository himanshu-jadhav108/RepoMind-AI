...
6. Observe the live features:
   ...
   - **Findings Workspace**: Switch tabs between Bugs, Security, Performance, Architecture, Documentation, and Feature Proposals. Filter by severity (`Critical`, `High`, `Medium`, `Low`). Inspect explainability fields (`Reasoning`, `Confidence %`, `AST Evidence Code Block`).
...
- `test_analysis_toolkit.py`: Ephemeral Git cloning, AST symbol parser, NetworkX graph.

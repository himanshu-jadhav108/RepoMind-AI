<div align="center">
  ...
  <img src="frontend/public/RepoMind_AI_logo.jpeg" alt="RepoMind AI Logo" width="140" style="border-radius: 24px; box-shadow: 0 12px 35px rgba(99, 102, 241, 0.45);" />
...
<div align="center">
  <table>
    <tr>
      <td width="50%">
        <img src="docs/screenshots/Landing_Page.png" alt="Modern SaaS Landing Page" width="100%" />
        <p align="center"><b>1. Modern SaaS Landing Page</b></p>
      </td>
      <td width="50%">
        <img src="docs/screenshots/Live_Workspace.png" alt="Autonomous Multi-Agent Workspace" width="100%" />
        <p align="center"><b>2. Autonomous Multi-Agent Workspace</b></p>
      </td>
    </tr>
    <tr>
      <td width="50%">
        <img src="docs/screenshots/Graph_View.png" alt="3D WebGL Galaxy Graph Matrix" width="100%" />
        <p align="center"><b>3. 3D WebGL Galaxy Graph Matrix</b></p>
      </td>
      <td width="50%">
        <img src="docs/screenshots/Tree_View.png" alt="2D Hierarchical ER Tree View" width="100%" />
        <p align="center"><b>4. 2D Hierarchical ER Tree View</b></p>
      </td>
    </tr>
    <tr>
      <td width="50%">
        <img src="docs/screenshots/Layer_View.png" alt="Clean Architecture Swimlanes" width="100%" />
        <p align="center"><b>5. Clean Architecture Swimlanes</b></p>
      </td>
      <td width="50%">
        <img src="docs/screenshots/Agent_Findings.png" alt="Multi-Agent Findings & Severity Triage" width="100%" />
        <p align="center"><b>6. Multi-Agent Findings & Severity Triage</b></p>
      </td>
    </tr>
    <tr>
      <td width="50%">
        <img src="docs/screenshots/Enginner_Review_Meeting.png" alt="Interactive Engineering Review Meeting" width="100%" />
        <p align="center"><b>7. Interactive Engineering Review Meeting</b></p>
      </td>
      <td width="50%">
        <img src="docs/screenshots/Smart_Learning.png" alt="Smart Learning Agent & Code Walkthrough" width="100%" />
        <p align="center"><b>8. Smart Learning Agent & Code Walkthrough</b></p>
      </td>
    </tr>
    <tr>
      <td width="50%">
        <img src="docs/screenshots/Repository_copilot.png" alt="Interactive Repository Copilot" width="100%" />
        <p align="center"><b>9. Interactive Repository Copilot</b></p>
      </td>
      <td width="50%">
        <img src="docs/screenshots/Audit_Report.png" alt="Executive Audit Report Export Center" width="100%" />
...
%%{init: {'theme': 'dark'}}%%
graph TD
    classDef userFill fill:#4338ca,stroke:#818cf8,stroke-width:3px,color:#ffffff;
    classDef ingestFill fill:#0284c7,stroke:#38bdf8,stroke-width:2px,color:#ffffff;
    classDef plannerFill fill:#7e22ce,stroke:#c084fc,stroke-width:2px,color:#ffffff;
    classDef analyzerFill fill:#3730a3,stroke:#818cf8,stroke-width:2px,color:#ffffff;
    classDef architectFill fill:#1d4ed8,stroke:#60a5fa,stroke-width:2px,color:#ffffff;
    classDef bughunterFill fill:#b45309,stroke:#fbbf24,stroke-width:2px,color:#ffffff;
    classDef securityFill fill:#047857,stroke:#34d399,stroke-width:2px,color:#ffffff;
    classDef perfFill fill:#c2410c,stroke:#fb923c,stroke-width:2px,color:#ffffff;
    classDef docsFill fill:#0369a1,stroke:#38bdf8,stroke-width:2px,color:#ffffff;
    classDef reviewerFill fill:#6d28d9,stroke:#e879f9,stroke-width:3px,color:#ffffff;
    classDef learningFill fill:#0f766e,stroke:#2dd4bf,stroke-width:2px,color:#ffffff;
    classDef suggestionFill fill:#be123c,stroke:#fb7185,stroke-width:2px,color:#ffffff;
    ...
    Analyzer --> BugHunter[4. Bug Hunter Agent]:::bughunterFill
    Analyzer --> Security[5. Security Agent]:::securityFill
    Analyzer --> Perf[6. Performance Agent]:::perfFill
    Analyzer --> Docs[7. Documentation Agent]:::docsFill

    Architect --> Reviewer[8. Reviewer Agent Loop]:::reviewerFill
    BugHunter --> Reviewer
    Security --> Reviewer
    Perf --> Reviewer
    Docs --> Reviewer

    Reviewer --> Learning[9. Learning Agent Walkthrough]:::learningFill
    Reviewer --> Suggestion[10. Feature Suggestion Agent]:::suggestionFill

    Learning --> Workspace([Live Engineering Workspace]):::outputFill
...
%%{init: {'theme': 'dark'}}%%
flowchart TD
    classDef client fill:#312e81,stroke:#818cf8,stroke-width:2px,color:#ffffff;
    ...
    subgraph ClientLayer ["Frontend Layer (Next.js 14 App Router)"]
        UI["Landing Page / Input Form"]:::client
        Dashboard["Multi-Agent Live Dashboard"]:::client
        Graph3D["3D WebGL Galaxy Graph"]:::client
        ReportView["Executive Report Generator"]:::client
    end

    subgraph APILayer ["API Layer (FastAPI Microservices)"]
        Router["API Router / Router V1"]:::api
        IngestService["Repo Ingestion Service"]:::api
        AnalysisService["Analysis Engine Service"]:::api
        ReportService["Report Generation Service"]:::api
    end
    ...
    subgraph DataLayer ["Data & Storage Layer"]
        Supabase[("Supabase Postgres DB")]:::db
        InMemory[("In-Memory Local Fallback Store")]:::db
    end
...
sequenceDiagram
    autonumber
    box rgba(99, 102, 241, 0.15) Client & User Interface
    actor Developer as 👨‍💻 User / Reviewer
    participant NextJS as ⚡ Next.js 14 Client
    end

    box rgba(14, 165, 233, 0.15) API & SSE Gateway
    participant FastAPI as 🚀 FastAPI Backend
    participant SSE as 📡 SSE Stream Endpoint
    end

    box rgba(168, 85, 247, 0.15) Autonomous Engine & AI Router
    participant LangGraph as 🤖 LangGraph Pipeline
    participant LLM as 🧠 LLM Provider Router
    end

    Developer->>NextJS: Submits GitHub Repo URL
    NextJS->>FastAPI: POST /api/v1/repos
    FastAPI-->>NextJS: Returns repo_id
    NextJS->>FastAPI: POST /api/v1/analysis/run
    FastAPI-->>NextJS: Returns run_id & status: queued

    par Real-Time SSE Updates
        NextJS->>SSE: GET /api/v1/analysis/run/{run_id}/stream
    ...
    and Async State Graph Execution
        ...
        LangGraph-->>FastAPI: Save Final State & Graph Data
    end

    NextJS->>FastAPI: GET /api/v1/analysis/run/{run_id}/graph
    FastAPI-->>NextJS: Returns 2D / 3D NetworkX Graph Data
    NextJS->>Developer: Renders Interactive 3D Galaxy Workspace
```
...
flowchart LR
    ...
    classDef db fill:#064e3b,stroke:#34d399,stroke-width:2px,color:#ffffff;
    classDef llm fill:#4c1d95,stroke:#c084fc,stroke-width:2px,color:#ffffff;

    subgraph Vercel ["Vercel Edge Platform"]
        NextFrontend["Next.js 14 Production Build<br/>(Standalone SSR Runner)"]:::vercel
    end

    subgraph Render ["Render Cloud Platform"]
        FastAPIBackend["FastAPI Server<br/>(Uvicorn ASGI Worker)"]:::render
    end

    subgraph Database ["Cloud Persistence"]
        SupabaseDB[("Supabase Postgres<br/>Storage")]:::db
    end

    subgraph LLMProviders ["AI Provider Mesh"]
        Gemini["Google Gemini API"]:::llm
        Groq["Groq Llama-3 API"]:::llm
        OpenAI["OpenAI GPT-4o API"]:::llm
    end

    NextFrontend -- REST API / SSE Stream --> FastAPIBackend
    FastAPIBackend -- Database Queries --> SupabaseDB
    FastAPIBackend -- Multi-Agent Prompts --> LLMProviders
```
